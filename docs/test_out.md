...
- 基于 Chromium 150.0.7871.0 (ChromiumFish 基线 a64c8adb6a) + ChromiumFish 21 个反指纹补丁
...
`
browser/metachat-patches/
├── 0002-switches.patch              # --metachat-mode 启动参数 + 命令拦截黑白名单
└── 0004-profile-seed.patch          # per-profile persona seed（通过 AppendExtraCommandLineSwitches 注入）

说明：品牌更名直接在 ChromiumFish 的 branding.patch 中改为 MetaChat（无需独立补丁）
      原生 UI 隐藏直接在 ChromiumFish 的 chrome-ui.patch 中加入可配置 allowlist/blocklist（无需独立补丁）
      overlay/webui/cdp/ 等功能在 UI 层通过 chrome://metachat/ WebUI 实现，无需 C++ 补丁
`
